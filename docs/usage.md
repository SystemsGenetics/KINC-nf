# systemsgenetics/kinc-nf: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/kinc/usage](https://nf-co.re/kinc/usage)
...
```console
nextflow run systemsgenetics/kinc-nf --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull systemsgenetics/kinc-nf
```
...
First, go to the [systemsgenetics/kinc-nf releases page](https://github.com/systemsgenetics/kinc-nf/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
